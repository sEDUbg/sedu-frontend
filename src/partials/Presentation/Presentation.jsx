...
const Presentation = ({ post, like, dislike, token }) => {
  ...
  useEffect(() => {
    ...
    if (load) {
      ...
      axios.post(`https://monkfish-app-swhuo.ondigitalocean.app/api/post/react/${token}/${reaction}`,
  ...
  useEffect(() => {
    ...
    //     if (userId in arr) {
    //       setReaction(
    //         arr[userId] == 1 ? "like" : arr[userId] == -1 ? "dislike" : ""
    //       );
    //     }
    //   }
    ...
    axios.get(`https://monkfish-app-swhuo.ondigitalocean.app/api/post/get/reaction/${token}`, { withCredentials: true })
      .then((res) => {
        console.log(res);
        setReaction(res.data.reaction == 1 ? "like" : res.data.reaction == -1 ? "dislike" : "");
        setLoad(true);
      })
    setLikes(like);
    setDislikes(dislike);
      ...
      <div className="presentation__content flex-1 rounded-2xl m-3 md:m-0 md:ml-10 md:my-10">
        <div className="presentation__content-thumbnail rounded-2xl overflow-hidden aspect-[17/10] bg-gray-100 dark:bg-slate-900 dark:border-slate-800">
          {/* <DocViewer
            ...
            documents={docs}
            config={docs_conf}
            className="rounded-2xl aspect-[17/10]"
          /> */}
        </div>
        <div className="presentation__content-title flex items-center justify-between bg-gray-100 border dark:border-slate-800 dark:bg-slate-900 rounded-2xl p-3 mt-5">
          ...
          <div className="presentation__content-title-actions flex items-center space-x-3">
            <div
              ...
              id="like"
              className="presentation__content-title-action flex items-center space-x-2 dark:text-white bg-gray-200 border border-gray-300 dark:border-slate-700 dark:bg-slate-800 p-2 rounded-full cursor-pointer"
            >
              {reaction == "like" ? (
                <FontAwesome.FaThumbsUp />
              ) : (
                <FontAwesome.FaRegThumbsUp />
              )}
              <p className="user-select: none">{likes || "0"}</p>
            </div>
            <div
            ...
            >
              ...
              <p className="user-select: none">{dislikes || "0"}</p>
            </div>
            <a
              onClick={() =>
                updateDoc(docRef, { "info.stats.downloads": increment(1) })
              }
              // href={link}
              target="_blank"
              rel="noopener noreferrer"
              className="presentation__content-title-action flex items-center dark:text-white bg-gray-200 border border-gray-300 dark:border-slate-700 dark:bg-slate-800 p-3 rounded-full"
            >
